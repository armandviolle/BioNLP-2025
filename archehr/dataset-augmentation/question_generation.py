...
def get_all_few_shot_examples(df: pd.DataFrame) -> List[dict]:
    ...
    for note, group in df.groupby("note_excerpt"):
        ...
        examples.append({"note_excerpt": note, "question": question})
    return examples
...
def generate_questions_summary(
    open_ai_key,
    i2b2_dataset_path,
    ...
    train_val_df["answers"] = train_val_df["answers"].apply(
        lambda x: x["text"][0] if isinstance(x, dict) and x["text"] else None
    )

    # Keep only rows where answer is actually found in context
    train_val_df["answer_in_context"] = train_val_df.apply(
        lambda row: str(row["answers"]).strip().lower()
        in str(row["context"]).strip().lower(),
        axis=1,
    )
    ...
    filtered_contexts = filtered_contexts[
        filtered_contexts.str.contains("history|hospital course", case=False, na=False)
    ]
    ...
    df = df[
        df["summary"].str.contains(
            "discharge medications:|history of present illness:", case=False, na=False
        )
    ]
    ...
    filtered_df["length_bin"] = pd.cut(
        filtered_df["word_count"], bins=bins, labels=labels
    )

    # Number of samples per bin
    samples_per_bin = 120

    sampled_rows = filtered_df.groupby("length_bin", group_keys=False).apply(
    ...
    df2 = convert_xml_df(
        xml_path="dev/archehr-qa.xml", key_json_path="dev/archehr-qa_key.json"
    )
    ...
    sampled_rows = sampled_rows[
        ~sampled_rows["normalized_summary"].isin(unique_df2["normalized_excerpt"])
    ...
    summaries_df = pd.concat(
        [
            i2b2_df[["summary", "source"]],
            emrqa_df[["summary", "source"]],
            mimic3_df[["summary", "source"]],
        ],
        ignore_index=True,
    )
    ...
    for i, row in tqdm(summaries_df.iterrows(), total=len(summaries_df)):
        ...
        prompt = build_prompt(
            new_input=note_excerpt, few_shot_examples=few_shot_examples
        )
    ...
    # Reorder columns
    final_df = final_df[["case_id", "note_excerpt", "generated_question", "source"]]
