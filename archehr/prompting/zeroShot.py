...
df["binary_relevance"] = df["relevance"].apply(
    lambda x: "essential" if x == "essential" else "not-relevant"
)
...
cases = df.groupby("case_id").agg({question_column: "first"}).reset_index()
...
llm = ChatOpenAI(
    model_name=model_name, temperature=temp_val, seed=seed_val, api_key=open_ai_api_key
)
...
def ask_question_and_get_ids(query, case_id):
    ...
    context = "\n".join([
        f"{doc.metadata['sentence_id']}: {doc.page_content}" for doc in case_docs
    ])
...
for _, row in tqdm(
    cases.iterrows(), total=cases.shape[0], desc="Evaluating and Generating"
):
    case_id = row["case_id"]
    ...
    gold_ids = (
        case_df[case_df["relevance"] == "essential"]["sentence_id"].astype(str).tolist()
    )  # gold
...
submission_json = [
    {"case_id": str(row["case_id"]), "answer": row["generated_answer"]}
