...
from utils import format_input, format_output, extract_sentences
...
def formatUserPrompt(
    ...
    if step == "gen-answer":
        ...
        patient_narrative = optional_input.find('patient_narrative').get_text(strip=True)
        ...
        single_sentences, sentences_prompt = extract_sentences(case=main_input, list_ids=optional_input)
        return f"# Patient Narrative\n{patient_narrative}\n\n# Clinician Question\n{clinician_question}\n\n# Relevant Sentences\n{sentences_prompt}"
    elif step == "format":
...
def main(
    ...
    for case in cases:
        ...
        # if case_id == "1":
        print(f"\n======== CASE ID {case_id}=========\n")
        seeds = np.random.choice(range(0, 43), size=args.n_seeds, replace=False).tolist()
        print(f"SEEDS: {seeds}")
        for s in seeds:
            ...
            print(f"ANSWER\n{answer1}\n")
            user_prompt2 = formatUserPrompt(main_input=answer1, step="retrieve-relevant", optional_input=case)
            answer2 = responseGPT(
                client=client,
                system_prompt=system_prompts["retrieve-relevant"], 
                user_prompt=formatUserPrompt(main_input=answer1, step="retrieve-relevant", optional_input=case), 
                model_name=args.model,
        ...
        paraphrases1 = responseGPT(
            ...
            user_prompt=formatUserPrompt(main_input=case, step="paraphrase", optional_input=pred_essentials), 
            model_name=args.model, 
            # FIX SEED ?
        )
        print("=== paraphrase1 ===")
        ...
        paraphrases2 = responseGPT(
            ...
            user_prompt=formatUserPrompt(main_input=paraphrases1, step="format", optional_input=(case, pred_essentials)), 
            model_name=args.model, 
            # FIX SEED ?
        )
        print("=== paraphrase2 ===")
        ...
        paraphrases3 = responseGPT(
            ...
            user_prompt=formatUserPrompt(main_input=paraphrases2, step="format", optional_input=(case, pred_essentials)), 
            model_name=args.model, 
            # FIX SEED ?
        )
        print("=== paraphrase2 ===")
